...
export default function CaseTypesPage() {
  ...
  const [editRow, setEditRow] = useState<CaseTypeRow | null>(null);
  ...
  function openEdit(r: CaseTypeRow) {
    setEditRow(r);
    setName(r.name);
  ...
  async function fetchRows() {
    ...
    try {
      ...
      setRows((data as CaseTypeRow[]) ?? []);
      setTotal(count ?? 0);
    } catch (e: any) {
      toast('error', e?.message ?? 'Failed to load case types.');
    } finally {
      setLoading(false);
    }
  }
  ...
  async function toggleActive(r: CaseTypeRow) {
    try {
      const { error } = await supabase.from('case_types').update({ is_active: !r.is_active }).eq('id', r.id);
      if (error) throw error;
  ...
  async function remove(r: CaseTypeRow) {
    const ok = confirm(`Delete "${r.name}"? This cannot be undone.`);
    if (!ok) return;

    try {
      const { error } = await supabase.from('case_types').delete().eq('id', r.id);
      if (error) throw error;
      toast('success', 'Deleted.');
      // adjust page if last row removed
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-semibold text-black">Case Types</h1>
      ...
      <div className="rounded-2xl border bg-white p-4 shadow-sm">
        ...
        {/* Filters drawer (simple inline panel) */}
        {filtersOpen && (
      ...
      <div className="overflow-hidden rounded-2xl border bg-white shadow-sm">
        ...
        <div className="flex flex-col gap-3 border-t px-5 py-4 md:flex-row md:items-center md:justify-between">
          <p className="text-sm text-gray-600">
            Showing {showingFrom} to {showingTo} of {total} case types
          </p>
      ...
      <Modal
        open={modalOpen}
        title={editRow ? 'Edit Case Type' : 'Add Case Type'}
